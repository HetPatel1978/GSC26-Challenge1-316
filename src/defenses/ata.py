...
def make_ata_aggregate(
    ...
    def aggregate_fn(results: List[Tuple[NDArrays, int]], global_params: NDArrays, round_num: int, **kwargs) -> NDArrays:
        ...
        # Stage 1: FLTrust-style whole-update cosine trust score.
        norms = [float(np.linalg.norm(df)) + 1e-12 for df in flat_deltas]
        trust_scores = [max(0.0, float(np.dot(g0_flat, df) / (g0_norm * n))) for df, n in zip(flat_deltas, norms)]
        ...
        agg_flat = np.where(flip_mask, -agg_flat, agg_flat)
